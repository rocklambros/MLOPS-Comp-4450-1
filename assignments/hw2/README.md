...
- Model filename. `sentiment_model.pkl` is the canonical model, loaded by `app.py` and copied
  into the image. `model.pkl` is a symlink to it, present only to match the filename shown in
  the Assignment 2 spec tree. The `model.pkl`-vs-`sentiment_model.pkl` mismatch is flagged, not
  silently resolved.
